 ...
 */
final class MongoDbEventStoreAdapterTransactionTest extends TestCase
{
    /**
     ...
     * @return Stream
     */
    private function getLongTestStream($fromVersion, $toVersion)
    {
        $generator = function ($fromVersion, $toVersion) {
            $version = $fromVersion - 1;

            while ($version++ <= $toVersion) {
                yield UserCreated::with(
                    ['name' => 'Max Mustermann', 'email' => 'contact@prooph.de'],
                    $version
                );
            }
        ...
        return $generator($fromVersion, $toVersion);
    }

    /**
     * @test
     */
    public function it_should_not_read_uncommited_events()
    {
        $streamName = new StreamName('Prooph\Model\User');
        $eventsCount = 2;
        $loops = 0;
        $maxLoopsPerStreamSize = 20;
        ...
        while ($loops++ < 20) {
            ...
            while (++$retries < $maxLoopsPerStreamSize) {

                $this->setUp();

                echo sprintf("testing with %d events\n ", $eventsCount);

                flush();

                $longStream = $this->getLongTestStream(2, $eventsCount);
                $this->adapter->create($this->getTestStream());

                $workerThread = new WorkerThread();

                //writing
                $this->adapter->appendTo($streamName, $longStream);
                ...
                $readEvents = $workerThread->getReadEvents();

                var_dump(count($readEvents));

                $this->assertTrue(
                    count($readEvents) === 1 || count($readEvents) === $eventsCount,
                    sprintf("should read 1 event or all %d events, actual %d events read", $eventsCount, count($readEvents)));
            }

            $eventsCount = $eventsCount * 2;
        }
    }
}
...
class WorkerThread extends \Thread
{
    ...
    public $readEvents;
    /**
     * @var \Prooph\EventStore\Adapter\MongoDb\MongoDbEventStoreAdapter
     */
    public $adapter;
    ...
    public function run()
    {
        try {
            ...
            $readEvents = iterator_to_array($stream->streamEvents());

            echo sprintf("read %d events", count($readEvents)) . "\n";
            flush();
        } catch (\Throwable $e) {
